...
describe('Follow Events', () => {
  ...
  it('Follow button should be SHOWN if user searched, found and unfollowed', () => {
    ...
    //type in search term 
    fireEvent.change(getByTestId("searchBar"), { target: { value: 'aron' } })

    //The button should exist, else, fail the test 
    expect(getByTestId('followUser'));
  });

  it('follow button calls followUser', () => {
  ...
  it('Unfollow button should be SHOWN if user searched, found and followed', () => {
    store = mockStore(initialState)
    const { getByTestId } = render(
      <Provider store={store}>
  ...
  it('unfollow button calls unfollowUser', () => {
  ...
  it('followUser() should reflect in Firestore', () => {
  ...
  it('unfollowUser() should reflect in Firestore', () => {
    const firebase = require('firebase'); // or import firebase from 'firebase';
    const db = firebase.firestore();
